...
- driver 驱动：触发或推动本窗口异动的事件，**同一事件簇的相关报道（首报 / 补充 / 后续）都标 driver**，不分主次
...
- contradictory 方向矛盾：新发生的**重大**事件，理论方向与窗口价格方向相反。**这是关系标签、不是因果标签**——只要事件量级达到「方向一致时就值得标 driver」的水平、发布时间与窗口吻合、而价格反着走，就标 contradictory；**不需要**证明它"导致"了反向走势，也不需要排除其他原因。被市场无视的重大消息本身就是高价值训练信号（已定价 / 仓位主导的证据），不要降级成 noise。两类典型：缓和消息发布后价格继续下跌；地缘升级密集发布而风险资产逆势上涨。与 noise 的分界：不重要 / 无关 / 事件早已发生且已定价 → noise；重要到「本该」引发同向反应却没有 → contradictory

窗口级 market_reaction_type（三选一，与 news_roles 联动：前两类必须有 driver，第三类必须没有）：
- macro_policy 宏观与政策预期：driver 是宏观数据公布 / 央行决议 / 官员讲话 / 财政与政策
- event_driven 事件驱动：driver 是宏观数据 / 政策**以外**的明确突发事件（地缘军事 / 制裁 / 监管 / 行业 / 标的专属如 ETF、交易所）
- no_news_driver 无新闻驱动：没有 driver——价格在动但候选里没有新增信息（情绪 / 仓位 / 技术位 / 无法归因都算这类）

窗口级 confidence（0-1）：0.8-1.0 = 新闻与价格方向、时间、资产高度匹配（no_news_driver 时 = 确认候选里没有信息）；0.5-0.8 = 可能是主因之一；<0.5 = 归因不确定。

**首要原则：宁可保守，不要乱标。**
- 默认所有候选都是 noise。**只有当某条新闻能给出强而具体的因果链条**（"X 事件直接导致 symbol 价格 Y"），才标 driver。
- 模糊关联、宏观背景文章、与该 symbol 仅有间接联系的新闻 → 保持 noise；行情综述 / 事后总结 → post_hoc_explanation；驱动事件的重复转述 / 后续报道 → 也标 driver（同事件簇）。**唯一例外**见下方 reference_changes 段：被跨资产签名确认的重大宏观 / 地缘**突发事件**，不算"间接联系"。
- 没把握时不给 driver，market_reaction_type 给 no_news_driver 并用 confidence 表达确定程度 —— 这份数据要拿去训练模型，假阳性会污染训练集。
- 下文条款中的「选入 / 不选」语义统一对应：**选入 = 标 driver；不选 = 保持 noise 或标 post_hoc_explanation / contradictory**。
...
- 窗口元数据附带同一时段宏观对标品种（纳指=NQ 期货（近 23 小时交易，美股收盘后仍有效）/ 原油 / 黄金 / 美债10Y / 美元指数 / BTC 等）的变动；null 表示该品种休市或无数据。美债10Y 以**基点**表示（+10.0bp = 收益率上行 0.10 个百分点），其余为涨跌百分比。
...
3. 没有 driver → market_reaction_type=no_news_driver，summary 写"无明显因果新闻"，confidence 表达"确认无信息"的程度；
4. 有 driver → 按驱动事件类型给 macro_policy 或 event_driven，给出 confidence，summary 用一句话写清楚因果链条。

只返回 JSON，不要 Markdown 标记，不要解释性正文。格式：
{
  "news_roles": {"<新闻id>": "driver|post_hoc_explanation|contradictory", ...},   // 只列非 noise 的候选；id 必须真实存在于候选列表
  "market_reaction_type": "macro_policy|event_driven|no_news_driver",
  "confidence": 0.85,                // 0-1
...
- driver 驱动：触发或推动该窗口异动的事件，**同一事件簇的相关报道（首报 / 补充 / 后续）都标 driver**，不分主次
...
窗口级 confidence（0-1）：0.8-1.0 = 高度匹配（no_news_driver 时 = 确认候选里没有信息）；0.5-0.8 = 可能是主因之一；<0.5 = 归因不确定。
...
**首要原则：宁可保守，不要乱标。**
- 默认所有候选都是 noise。**只有当某条新闻能给出强而具体的因果链条**（"X 事件直接导致 symbol 价格 Y"），才标 driver。
- 模糊关联、宏观背景、与该 symbol 仅有间接联系的新闻 → 保持 noise；行情综述 / 事后总结 → post_hoc_explanation；驱动事件的重复转述 / 后续报道 → 也标 driver（同事件簇）。**唯一例外**见下方 reference_changes 段：被跨资产签名确认的重大宏观 / 地缘**突发事件**，不算"间接联系"。
- 没把握时不给 driver，market_reaction_type 给 no_news_driver 并用 confidence 表达确定程度 —— 这份数据要拿去训练模型，假阳性会严重污染训练集。
...
- 每个窗口附带同一时段宏观对标品种（纳指=NQ 期货（近 23 小时交易，美股收盘后仍有效）/ 原油 / 黄金 / 美债10Y / 美元指数 / BTC 等）的变动；null 表示该品种休市或无数据。美债10Y 以**基点**表示（+10.0bp = 收益率上行 0.10 个百分点），其余为涨跌百分比。
...
只返回 JSON：
{
  "items": [
    {
      "window_id": int,
      "news_roles": {"<新闻id>": "driver|post_hoc_explanation|contradictory", ...},   // 只列非 noise；id 只能来自该 window 自己的 candidates
      "market_reaction_type": "macro_policy|event_driven|no_news_driver",
      "confidence": 0.85,                // 0-1
      "summary": "不超过 80 字的归因结论；无明显驱动时写'无明显因果新闻'即可",
...
# 用于按提示词版本切分自动标注数据。
ANNOTATION_PROMPT_VERSION = "v4-20260612"
