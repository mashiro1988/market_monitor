...
def _reference_changes_payload(refs: list[ReferenceChange]) -> dict[str, str | None]:
    """同期对标涨跌 → 喂给 reasoner 的紧凑 dict：{label: "+1.23%"}；无数据 None；标注品种本身不列。"""
    out: dict[str, str | None] = {}
    for ref in refs:
        if ref.is_self:
            continue
        out[ref.label] = f"{ref.pct:+.2f}%" if ref.pct is not None else None
    return out
...
- 看到模糊关联、事后总结、行情综述、宏观背景文章、与该 symbol 仅有间接联系的新闻，**全部不选**。
...
- 窗口元数据附带同一时段宏观对标品种（纳指 / 原油 / 黄金等）的涨跌幅；null 表示该品种休市或无数据。
- 用它判断本次异动是「标的独立行情」还是「宏观 / 风险事件共振」：
  - 标的与对标品种同向联动（如纳指、BTC 同跌）→ 优先在候选里找宏观、地缘、政策类**突发事件**；
  - 股指下跌 + 原油/黄金上涨，是典型的地缘冲突 / 供给冲击签名（军事行动、空袭、制裁、油轮遇袭等）；股指与原油同跌偏向衰退 / 需求担忧；
  - 对标品种基本走平、只有标的自己异动 → 找该资产**专属**新闻（如 BTC 的 ETF / 链上 / 监管事件），此时宏观新闻不可选。
...
判断步骤：
1. 先扫一遍候选新闻，列出**与该 symbol 在所述时段内有强直接因果关系**的新闻；
2. 如果第 1 步结果为空 → `no_clear_news=true, selected_news_ids=[]`，summary 写"无明显因果新闻"；
3. 否则把这些新闻 id 放进 selected_news_ids，summary 用一句话写清楚因果链条。
  ...
  "windows": [
    {
      ...
      "reference_changes": { "纳指": "+0.42%", "原油": "-1.10%", ... },   // 同期对标品种涨跌；null=休市/无数据
...
- 模糊关联、事后总结、行情综述、宏观背景、与该 symbol 仅有间接联系的新闻，**全部不选**。
...
- 每个窗口附带同一时段宏观对标品种（纳指 / 原油 / 黄金等）的涨跌幅；null 表示该品种休市或无数据。
- 用它判断该窗口异动是「标的独立行情」还是「宏观 / 风险事件共振」：
  - 标的与对标品种同向联动（如纳指、BTC 同跌）→ 优先在该窗口候选里找宏观、地缘、政策类**突发事件**；
  - 股指下跌 + 原油/黄金上涨，是典型的地缘冲突 / 供给冲击签名（军事行动、空袭、制裁、油轮遇袭等）；股指与原油同跌偏向衰退 / 需求担忧；
  - 对标品种基本走平、只有标的自己异动 → 找该资产**专属**新闻（如 BTC 的 ETF / 链上 / 监管事件），此时宏观新闻不可选。
- 突发地缘 / 军事 / 制裁类新闻通常**不会提到 symbol 本身**：只要事件足够重大、发布时间与窗口吻合、且跨资产签名一致，就应视为对股指 / 风险资产的**直接**触发选入，不要因为"新闻没提该品种"而当作间接关联排除。注意区分：**新发生的事件**（首次交火、首次空袭）可选；对既有局势的回顾、分析、评论文章仍然不选。
- 标注品种本身若在对标清单里（如标注纳指 NQ=F 的窗口），它**不会出现在 reference_changes 里**——其涨跌就是窗口自身的 change_pct，键缺失不是数据故障。
...
每个窗口的判断步骤：
1. 先扫一遍该窗口的候选新闻，列出**与该 symbol 在所述时段内有强直接因果关系**的新闻；
2. 如果第 1 步结果为空 → `no_clear_news=true, selected_news_ids=[]`，summary 写"无明显因果新闻"；
3. 否则把这些新闻 id 放进 selected_news_ids，summary 用一句话写清楚因果链条。
