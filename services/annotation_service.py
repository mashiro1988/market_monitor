...
- 同一事件的多条连续快讯（同一讲话/同一袭击的不同来源、不同侧面）属于一个事件簇：driver 给**市场实际反应对上的那一条**——发布时点紧贴窗口起点 / 加速段、发布后价格随即启动的优先。**价格反应检验**（2026-07-21）：更早发布但发布后价格没反应的报道（含首报），驱动地位已被价格证伪 → redundant；早报发布后价格**立即有部分反应**、后续详版才带来加速的，早报仍是 driver。多条时点都贴合时（time_bj 只有分钟精度，相差 ≤2 分钟即视为同时），**原始信源优先于转推**：同一句快讯的多源 / 多语言版本（内容互为翻译或转述）里，driver 给**原文首发**那条——海外事件以英文快讯源为原文，中文源（jin10 / 华尔街见闻等）的转译版**一律归 redundant，哪怕它发布更晚、内容更全**；中国本土事件反之以中文源为原文。簇里没有原文版本时才可把转译版标 driver。以上仍分不出，再退回"信息量最大"做末级裁决。其余报道标 redundant（不必只选 1-3 条、也不要降级成 noise）。
...
ANNOTATION_PROMPT_VERSION = "v16-20260721"
