...
- `reference_change_segments` 包含标注品种本身（如 BTC），这是和其它大类资产比较的基准；`reference_changes` 仍不包含标注品种本身，因为它不是自己的对标资产。
...
# 用于按提示词版本切分自动标注数据。
ANNOTATION_PROMPT_VERSION = "v10-20260702"
...
def export_training_jsonl(session: Session, days: int = 365, split: str = "train") -> list[str]:
    ...
    for row in rows:
        ...
        # 派生信号（correlations/三段）保持 compute-on-read（feat/onchain-market-overview）。
