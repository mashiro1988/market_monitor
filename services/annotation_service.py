...
def _window_signals_payload(session: Session, symbol: str,
    ...
    from services import window_signals
    correlations_by_symbol = _reference_correlations_for_window(session, symbol, window_start, window_end)
    labels_by_symbol = {sym: label for sym, label, _unit in _iter_reference_assets()}
    correlations = {
        labels_by_symbol[sym]: value
        for sym, value in correlations_by_symbol.items()
        if sym in labels_by_symbol
    }
...
你将收到：
1. 一段价格异动窗口的元数据（symbol、起止时间、价格变化百分比、阈值、同期对标品种涨跌 reference_changes，以及窗口 ±1h 相关性 correlations）。
...
**关于窗口派生信号（window 里的 correlations / trigger_move_start_bj / pre_window_move_pct）—— 重点判据**：
...
3. **相关性 = 在跟谁走**：`correlations` 是本品种与各对标在 ±1h 的相关系数。**|corr|≥0.7** 说明本品种在跟那个对标联动 → 优先找能驱动**那个相关品种**的突发事件新闻。反过来：一条新闻**只描述价格走势本身、给不出背后真实世界事件**的 → noise（大类资产联动由相关性解释，不由这种描述性新闻解释）。
4. **跨资产签名确认真伪**：方向对但直觉冲击不大、且价格反应滞后的新闻，别急着标 driver——要**整套跨资产签名对齐**才认。例：鹰派联储（"通胀在升、年内加息"）应看到 **BTC/纳指下行 + 美元指数走强 + 美债利率上行 同步**；若这些 corroborating 资产（看 reference_changes / correlations）**不配合**，就该怀疑不是这条在驱动，降级 noise。
...
**核心推理顺序（必须执行）**：
1. 先看 `correlations`：尤其是 BTC 窗口，先判断它是在跟纳指/日经/黄金/原油/美元/美债哪条通道走。若某资产 |corr|≥0.7，把它当作本窗口的主线索。
2. 围绕最相关资产找新闻：BTC 高相关纳指/日经时，优先找能驱动风险资产的宏观、利率、地缘或政策新闻；高相关黄金/美元/美债时，优先找避险、通胀、利率和流动性新闻；高相关原油时，优先找供给、地缘和库存/OPEC 新闻。
3. 再用 `reference_changes` 里的其它资产交叉验证：候选新闻必须能解释一条完整资产联动链。例：通胀风险下降 / 加息预期走弱，通常应看到美元指数或美债利率走弱，并可能推升黄金或风险资产；如果新闻含义与实际资产链条相反，就降级 noise。
4. 只有「相关资产新闻 + 其它资产验证 + 时间靠近触发段」三者合起来说得通，才标 driver；缺一项时宁可不标，用 confidence 表达不确定。
...
判断步骤：
1. 先看 correlations，找出本窗口最像在跟随的对标资产；没有高相关资产时，再退回 reference_changes 的同期方向。
2. 围绕最相关资产扫描候选新闻：共振时重点核对宏观 / 地缘 / 利率 / 政策类突发事件；独立行情时只考虑该资产专属新闻。
3. 用 reference_changes 里的其它资产做交叉验证，资产链条和新闻含义不一致时降级 noise；行情综述 / 收盘总结 / 解释性 / 方向相反 / 离题的消息一律默认 noise（不必单列）。
  ...
  "windows": [
    {
      ...
      "correlations": { "纳指": 0.82, "日经225": 0.76, "美元指数": -0.71, ... },                // ±1h 5min 收益率相关；缺失=样本不足/无波动
...
**关于每个窗口的派生信号（window 里的 correlations / trigger_move_start_bj / pre_window_move_pct）—— 重点判据**：
1. **没明显相关新闻就别标 driver**：默认所有候选 noise，只有能给出强而具体因果链条的才 driver。
...
3. **相关性 = 在跟谁走**：`correlations` 是该品种与各对标在 ±1h 的相关系数。**|corr|≥0.7** 说明在跟那个对标联动 → 优先找能驱动**那个相关品种**的突发事件新闻。反过来：**只描述价格走势本身、给不出背后真实世界事件**的新闻 → noise（大类联动由相关性解释）。
...
**核心推理顺序（必须执行）**：
1. 先看 `correlations`：尤其是 BTC 窗口，先判断它是在跟纳指/日经/黄金/原油/美元/美债哪条通道走。若某资产 |corr|≥0.7，把它当作该窗口的主线索。
2. 围绕最相关资产找新闻：BTC 高相关纳指/日经时，优先找能驱动风险资产的宏观、利率、地缘或政策新闻；高相关黄金/美元/美债时，优先找避险、通胀、利率和流动性新闻；高相关原油时，优先找供给、地缘和库存/OPEC 新闻。
3. 再用 `reference_changes` 里的其它资产交叉验证：候选新闻必须能解释一条完整资产联动链。例：通胀风险下降 / 加息预期走弱，通常应看到美元指数或美债利率走弱，并可能推升黄金或风险资产；如果新闻含义与实际资产链条相反，就降级 noise。
4. 只有「相关资产新闻 + 其它资产验证 + 时间靠近触发段」三者合起来说得通，才标 driver；缺一项时宁可不标，用 confidence 表达不确定。
...
每个窗口的判断步骤：
1. 先看该窗口的 correlations，找出最像在跟随的对标资产；没有高相关资产时，再退回 reference_changes 的同期方向。
2. 围绕最相关资产扫描候选新闻：共振时重点核对宏观 / 地缘 / 利率 / 政策类突发事件；独立行情时只考虑该资产专属新闻。
3. 用 reference_changes 里的其它资产做交叉验证，资产链条和新闻含义不一致时降级 noise；行情综述 / 收盘总结 / 解释性 / 方向相反 / 离题的消息一律默认 noise（不必单列）。
4. 没有 driver → summary 写"无明显因果新闻"，confidence 表达"确认无信息"的程度；有 driver → 给出 confidence，summary 用一句话写清楚因果链条。
