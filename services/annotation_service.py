...
def _window_signals_payload(session: Session, symbol: str,
    ...
    v14 起 machine_class 退出——机器归因不准，不再作为推理起点喂给模型）：
    - s_scores / max_ref / sync_ref_count：共振分证据链。s = 段窗内 rolling 曲线
      |S| 峰值（与标注页曲线同一个数，"所见即所判"）；含符号（美元反向为常态），判级用 |S|。
    - reference_change_segments：固定三段方向链（含标注品种本身），交叉验证用。
    - trigger_move_start_bj / _pct：窗口内加速最陡的 5min bar（对时锚一律用窗口起点=0.3 档启动点）。
...
def _effective_hours(hours: int | None) -> int | None:
    """回溯小时数归一：>0 → clamp 到 [1, 720]；0 / 负 / None → None = 全量（不设回溯边界）。
    2026-07-19 标注页撤掉回溯筛选后前端固定传 0。"""
...
你将收到：
1. 一段价格异动窗口的元数据（symbol、起止时间、价格变化百分比、阈值、同期对标品种涨跌 reference_changes、固定三段方向链 reference_change_segments，以及共振分证据 s_scores / max_ref / sync_ref_count）。
2. 该窗口前后一段时间内的候选新闻条目（含 id、北京时间、来源、LLM 评分、标题、内容片段）。

**标签体系**：

每条新闻的 causal_role（输出里**只列非 noise** 的候选，未列出的一律默认 noise）：
- driver 驱动：触发或推动本窗口异动的**主**事件——同一事件簇里**市场实际反应对上的那一条**标 driver（选法见下方事件簇条款）
...
**关于窗口派生信号（window 里的 s_scores / max_ref / trigger_move_start_bj / pre_window_move_pct）—— 重点判据**：
1. **没明显相关新闻就别标 driver**：默认所有候选 noise，只有能给出强而具体因果链条的才 driver。
2. **对时锚 = 窗口起点（0.3 档启动点）**：窗口即行为段，`window_start` 就是价格开始异动的时点（15min 净幅触及 0.3 档才成段，起点前没有平段）。找 driver 以**窗口起点为主要对时锚**：起点前 ~60min 到窗口早段的新闻都算及时（消息传导到 BTC 常滞后十到几十分钟）。`trigger_move_start_bj` 只是窗口内**加速最陡的 5min bar**（0.3 启动 → 0.5/0.8 加速的路径点）——新闻早于它几十分钟但接近窗口起点，属正常传导节奏，**不构成排除理由，也不要为此压低置信度**。它为 null 时退回看整段。但传导滞后是**容忍项，不是偏好项**（2026-07-21）：同等条件下 driver 优先给**紧贴窗口起点 / 加速段之前**的那条；若一条新闻发布后 20-30 分钟内，本品种与最强参照都**没有朝事件方向的可见反应**、直到另一条同类消息出现价格才启动——前一条的驱动主张已被价格证伪，不标 driver（同簇时归 redundant）。
3. **共振分 = 在跟谁走**：`s_scores` 是本品种与各对标的**共振分 S**（−1~+1：本品种使劲的时刻，对标朝同方向跟了几成，异动点加权；取值 = 段窗内 rolling 曲线的 |S| **峰值**，与标注页联动曲线所见同数）。**符号只表示方向关系**（美元指数为负 = 正常反向联动），**判联动强弱一律用 |S|**：`max_ref` 给出最强参照——**max|S| ≥ 0.5 = 宏观耦合（共振）**，优先找能驱动**最强参照**的突发事件新闻；0.3–0.5 = 弱共振（仅参考）；< 0.3 = 独立行情。每个分数附 `ess`（证据厚度：< 5 = 分数只靠一两根 K 线撑起，**证据薄**，据此下调 confidence）和 `coverage`（参照覆盖）。`sync_ref_count` = |S|≥0.3 的参照个数（区分"只跟单一资产走"与"全市场共振"）。一条新闻**只描述价格走势本身、给不出背后真实世界事件**的 → noise（大类联动由共振分解释，不由这种描述性新闻解释）。
...
**核心推理顺序（必须执行）**：
1. 先看 `max_ref` 定行情性质：max|S| ≥ 0.5 → 宏观耦合（候选里有量级匹配的新闻 = 新闻驱动；没有 = 纯共振，别硬凑 driver）；< 0.3 → 独立行情（只考虑该资产专属新闻）；`s_scores` 为空 = **无对照（宏观休市）≠ 无宏观新闻**——周末突发事件照样可标 driver，按「对标不可用」条款做纯事件判断。ESS < 5 或 coverage 低时在 reasoning 里明示证据薄、下调 confidence。
...
4. 只有「最强参照的新闻 + 其它资产验证 + 时间贴合对时锚（窗口起点）」三者合起来说得通，才标 driver；缺一项时宁可不标，用 confidence 表达不确定。
...
**关于新闻时间戳的关键说明**：候选新闻的 time_bj 是新闻**发布时间**（精确到秒），不是事件发生时间。中文财经源（华尔街见闻、jin10）和翻译类英文源经常把欧美时段的事件延迟 5-30 分钟才推送到国内。所以：
...
- 同一事件的多条连续快讯（同一讲话/同一袭击的不同来源、不同侧面）属于一个事件簇：driver 给**市场实际反应对上的那一条**——发布时点紧贴窗口起点 / 加速段、发布后价格随即启动的优先。**价格反应检验**（2026-07-21）：更早发布但发布后价格没反应的报道（含首报），驱动地位已被价格证伪 → redundant；早报发布后价格**立即有部分反应**、后续详版才带来加速的，早报仍是 driver。同一句快讯的多源 / 多语言版本（内容互为翻译或转述，相差几秒到一两分钟，价格来不及在其间区分反应）：driver 给 **time_bj 秒级最早**的那条——按"信息最先进入市场"裁决，**不要按信源语言、名气或内容详略猜"原文"**；其余版本一律归 redundant，哪怕发布更晚、内容更全。秒级时间完全相同的极端情况任选其一即可，不必按信息量纠结。其余报道标 redundant（不必只选 1-3 条、也不要降级成 noise）。

判断步骤：
1. 先看 max_ref 定行情性质（共振、弱共振、独立、无对照）；无对照时新闻命中检查照常做，不得用"参照没动"排除宏观新闻。
...
- driver 驱动：触发或推动该窗口异动的**主**事件——同一事件簇里**市场实际反应对上的那一条**标 driver（选法见下方事件簇条款）
- redundant 同簇冗余：与 driver 同一事件簇的**其它**相关报道（首报 / 补充 / 后续 / 不同来源转述）→ 标 redundant，相关但非主代表，**不要降级成 noise**（训练时不当负样本）
  ...
  "windows": [
    {
      ...
      "candidates": [{ "id": int, "time_bj"（精确到秒）, "source", "llm_score", ... }, ...]
    },
    ...
  ]
}
...
**关于每个窗口的派生信号（window 里的 s_scores / max_ref / trigger_move_start_bj / pre_window_move_pct）—— 重点判据**：
1. **没明显相关新闻就别标 driver**：默认所有候选 noise，只有能给出强而具体因果链条的才 driver。
2. **对时锚 = 窗口起点（0.3 档启动点）**：窗口即行为段，`window_start` 就是价格开始异动的时点（15min 净幅触及 0.3 档才成段，起点前没有平段）。找 driver 以**窗口起点为主要对时锚**：起点前 ~60min 到窗口早段的新闻都算及时（消息传导到 BTC 常滞后十到几十分钟）。`trigger_move_start_bj` 只是该窗口内**加速最陡的 5min bar**（0.3 启动 → 0.5/0.8 加速的路径点）——新闻早于它几十分钟但接近窗口起点，属正常传导节奏，**不构成排除理由，也不要为此压低置信度**。它为 null 时退回看整段。但传导滞后是**容忍项，不是偏好项**（2026-07-21）：同等条件下 driver 优先给**紧贴窗口起点 / 加速段之前**的那条；若一条新闻发布后 20-30 分钟内，本品种与最强参照都**没有朝事件方向的可见反应**、直到另一条同类消息出现价格才启动——前一条的驱动主张已被价格证伪，不标 driver（同簇时归 redundant）。
3. **共振分 = 在跟谁走**：`s_scores` 是该品种与各对标的**共振分 S**（−1~+1，异动点加权的方向跟随度；取值 = 段窗内 rolling 曲线的 |S| **峰值**，所见即所判）。**符号只表示方向关系**（美元指数为负 = 正常反向联动），**判联动强弱一律用 |S|**：`max_ref` 给最强参照——**max|S| ≥ 0.5 = 宏观耦合**，优先找能驱动**最强参照**的突发事件新闻；0.3–0.5 = 弱共振（仅参考）；< 0.3 = 独立行情。`ess` < 5 = **证据薄**（分数只靠一两根 K 线，下调 confidence）；`sync_ref_count` 区分"跟单一资产"与"全市场共振"。**只描述价格走势本身、给不出背后真实世界事件**的新闻 → noise（大类联动由共振分解释）。
...
**核心推理顺序（必须执行）**：
1. 先看该窗口的 `max_ref` 定行情性质：max|S| ≥ 0.5 → 宏观耦合（有量级匹配的新闻 = 新闻驱动；没有 = 纯共振，别硬凑 driver）；< 0.3 → 独立行情（只考虑该资产专属新闻）；`s_scores` 为空 = **无对照（宏观休市）≠ 无宏观新闻**——周末突发照样可标 driver，按「对标不可用」条款做纯事件判断。ESS < 5 或 coverage 低时在 reasoning 里明示证据薄、下调 confidence。
...
每个窗口的判断步骤：
1. 先看该窗口的 max_ref 定行情性质（共振、弱共振、独立、无对照）；无对照时新闻命中检查照常做，不得用"参照没动"排除宏观新闻。
2. 围绕最强参照扫描候选新闻：共振时重点核对宏观 / 地缘 / 利率 / 政策类突发事件；独立行情时只考虑该资产专属新闻。
3. 用 reference_changes 与三段方向链做交叉验证，资产链条和新闻含义不一致时降级 noise；行情综述 / 收盘总结 / 解释性 / 方向相反 / 离题的消息一律默认 noise（不必单列）。
4. 没有 driver → summary 写"无明显因果新闻"，confidence 表达"确认无信息"的程度；有 driver → 给出 confidence，summary 用一句话写清楚因果链条。
...
- 推理优先级：先 `max_ref` 定行情性质，再围绕最强参照找新闻，最后用三段方向链和其它大类资产交叉验证；方向链与新闻含义相反时降级 noise，**证据薄（ess<5）时下调 confidence**。
...
# 用于按提示词版本切分自动标注数据。
# v14（2026-07-19）：machine_class 退出 payload 与提示词（机器归因不准，不再作推理起点）。
# v15（2026-07-21）：driver 对时收紧——同簇主代表从"信息量最大"改为"市场实际反应对上的那条"；
# 新增价格反应检验（发布后 20-30min 无反应、直到同类消息出现才启动 → 前一条被证伪归 redundant）。
# v16（2026-07-21）：同簇时点分不出先后时，原始信源优先于转推（外电原文 > 转译复述），"信息量最大"退居末级裁决。
# v17（2026-07-21）：撤销 v16 的"按语言猜原文"（用户否决——英文≠原文没有道理）；candidates.time_bj 加秒，
# 同簇互译/转述版本一律 driver 给秒级最早那条（信息最先进入市场），"信息量"裁决彻底退场。
ANNOTATION_PROMPT_VERSION = "v17-20260721"
...
def load_price_windows(
    ...
    if eff_hours is not None:
        ...
        first_start = (
            ...
            .filter(BehaviorSegment.symbol == symbol, BehaviorSegment.tier_idx >= 1)
            .scalar()
        )
...
def list_annotations(
    ...
    # 时代守卫（2026-07-19 全量回溯后补）：早于该品种最早行为段的老标注（当时窗口源
    # 还不是行为段）本就匹配不到任何当前窗口，永久橙标没有意义 → 不参与 needs_review。
    cur_ids_by_symbol: dict[str, set[int]] = {}
    s_scores_by_ann_by_symbol: dict[str, dict[int, dict]] = {}
    ...
    def _era_anchor(sym: str) -> datetime | None:
        if sym not in era_anchor_by_symbol:
            from models.behavior import BehaviorSegment
            era_anchor_by_symbol[sym] = (
                session.query(func.min(BehaviorSegment.start_dt))
                .filter(BehaviorSegment.symbol == sym, BehaviorSegment.tier_idx >= 1)
                .scalar()
            )
        return era_anchor_by_symbol[sym]

    # 行内新闻摘要（2026-07-19）：driver / 同簇冗余的标题直接进列表行，免开详情逐条查。
    roles_by_row: dict[int, dict[int, str]] = {row.id: _parse_news_roles(row.news_roles) for row in rows}
    all_news_ids = {nid for roles in roles_by_row.values() for nid in roles}
    news_by_id = (
        {n.id: n for n in session.query(NewsItem).filter(NewsItem.id.in_(all_news_ids)).all()}
        if all_news_ids else {}
    )

    def _news_briefs(row_id: int) -> list[AnnotationNewsBrief]:
        briefs: list[AnnotationNewsBrief] = []
        for nid, role in roles_by_row.get(row_id, {}).items():
            n = news_by_id.get(nid)
            if n is None:
                continue
            briefs.append(AnnotationNewsBrief(
                id=nid, role=role, title=(n.title or "")[:160],
                time_bj=(n.timestamp + timedelta(hours=8)).strftime("%m-%d %H:%M") if n.timestamp else None,
            ))
        briefs.sort(key=lambda b: (0 if b.role == "driver" else 1, b.time_bj or ""))
        return briefs
    ...
    for row in rows:
        ...
        items.append(AnnotationListItem(
            ...
            news_briefs=_news_briefs(row.id),
...
def _build_auto_annotate_user_payload(
    ...
    for row in candidate_news:
        items.append({
            "id": row.id,
            "time_bj": (row.timestamp + timedelta(hours=8)).strftime("%Y-%m-%d %H:%M:%S") if row.timestamp else None,
            "source": row.source,
...
def export_training_jsonl(session: Session, days: int = 365, split: str = "train") -> list[str]:
    ...
    for row in rows:
        ...
        for nid in cand_ids:
            ...
            candidates.append({
                "id": nid,
                "time_bj": (n.timestamp + timedelta(hours=8)).strftime("%Y-%m-%d %H:%M:%S") if n and n.timestamp else None,
