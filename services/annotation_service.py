...
2. **对时锚 = 窗口起点（0.3 档启动点）**：窗口即行为段，`window_start` 就是价格开始异动的时点（15min 净幅触及 0.3 档才成段，起点前没有平段）。找 driver 以**窗口起点为主要对时锚**：起点前 ~60min 到窗口早段的新闻都算及时（消息传导到 BTC 常滞后十到几十分钟）。`trigger_move_start_bj` 只是窗口内**加速最陡的 5min bar**（0.3 启动 → 0.5/0.8 加速的路径点）——新闻早于它几十分钟但接近窗口起点，属正常传导节奏，**不构成排除理由，也不要为此压低置信度**。它为 null 时退回看整段。
...
1. **没明显相关新闻就别标 driver**：默认所有候选 noise，只有能给出强而具体因果链条的才 driver。
2. **对时锚 = 窗口起点（0.3 档启动点）**：窗口即行为段，`window_start` 就是价格开始异动的时点（15min 净幅触及 0.3 档才成段，起点前没有平段）。找 driver 以**窗口起点为主要对时锚**：起点前 ~60min 到窗口早段的新闻都算及时（消息传导到 BTC 常滞后十到几十分钟）。`trigger_move_start_bj` 只是该窗口内**加速最陡的 5min bar**（0.3 启动 → 0.5/0.8 加速的路径点）——新闻早于它几十分钟但接近窗口起点，属正常传导节奏，**不构成排除理由，也不要为此压低置信度**。它为 null 时退回看整段。
...
ANNOTATION_PROMPT_VERSION = "v14-20260719"
