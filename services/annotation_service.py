...
- 模糊关联、宏观背景文章、与该 symbol 仅有间接联系的新闻 → 保持 noise；行情综述 / 事后总结 / 解释性、离题、方向相反的消息 → 一律默认 noise；驱动事件的重复转述 / 后续报道 → 也标 driver（同事件簇）。**唯一例外**见下方 reference_changes 段：被跨资产签名确认的重大宏观 / 地缘**突发事件**，不算"间接联系"。
- 没把握时不给 driver，market_reaction_type 给 no_news_driver 并用 confidence 表达确定程度 —— 这份数据要拿去训练模型，假阳性会污染训练集。
- 下文条款中的「选入 / 不选」语义统一对应：**选入 = 标 driver；不选 = 保持 noise**（综述 / 解释 / 离题 / 方向相反一律默认 noise，不单列）。
...
2. 再扫一遍候选新闻，给非 noise 的条目标注 causal_role——第 1 步判为共振时，重点核对宏观 / 地缘 / 政策类突发事件；判为独立行情时，只考虑该资产专属新闻；行情综述 / 收盘总结 / 解释性 / 方向相反 / 离题的消息一律默认 noise（不必单列）；
...
{
  "news_roles": {"<新闻id>": "driver", ...},   // 只列非 noise 的候选；id 必须真实存在于候选列表
...
**首要原则：宁可保守，不要乱标。**
- 默认所有候选都是 noise。**只有当某条新闻能给出强而具体的因果链条**（"X 事件直接导致 symbol 价格 Y"），才标 driver。
- 模糊关联、宏观背景、与该 symbol 仅有间接联系的新闻 → 保持 noise；行情综述 / 事后总结 / 解释性、离题、方向相反的消息 → 一律默认 noise；驱动事件的重复转述 / 后续报道 → 也标 driver（同事件簇）。**唯一例外**见下方 reference_changes 段：被跨资产签名确认的重大宏观 / 地缘**突发事件**，不算"间接联系"。
...
2. 再扫一遍该窗口的候选新闻，给非 noise 的条目标注 causal_role——第 1 步判为共振时，重点核对宏观 / 地缘 / 政策类突发事件；判为独立行情时，只考虑该资产专属新闻；行情综述 / 收盘总结 / 解释性 / 方向相反 / 离题的消息一律默认 noise（不必单列）；
3. 没有 driver → market_reaction_type=no_news_driver，summary 写"无明显因果新闻"，confidence 表达"确认无信息"的程度；
4. 有 driver → 按驱动事件类型给 macro_policy 或 event_driven，给出 confidence，summary 用一句话写清楚因果链条。
...
只返回 JSON：
{
  "items": [
    {
      "window_id": int,
      "news_roles": {"<新闻id>": "driver", ...},   // 只列非 noise；id 只能来自该 window 自己的 candidates
      "market_reaction_type": "macro_policy|event_driven|no_news_driver",
      "confidence": 0.85,                // 0-1
      "summary": "不超过 80 字的归因结论；无明显驱动时写'无明显因果新闻'即可",
...
# 用于按提示词版本切分自动标注数据。
ANNOTATION_PROMPT_VERSION = "v5-20260622"
