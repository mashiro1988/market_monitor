...
给每条候选新闻标注**因果角色（causal_role）**，并判断本窗口的**市场反应类型（market_reaction_type）**与置信度。
...
- primary_driver 主驱动：直接触发本窗口异动的事件
- secondary_driver 次驱动：辅助驱动，或主驱动同一事件簇的补充 / 后续报道
- amplifier 放大器：放大既有趋势但非起因
- noise 噪音：与本窗口异动无关（默认值，**不要写进输出**）
- post_hoc_explanation 事后解释：价格先动、新闻找理由（"X 短线下挫，因 Y"类行情综述、收盘总结）
- contradictory 方向矛盾：新闻方向与价格反应相反（如缓和消息发布后价格继续下跌）

窗口级 market_reaction_type（八选一）：
fundamental_repricing 基本面重估 / policy_expectation_shift 政策预期变化 / liquidity_shock 流动性冲击 /
risk_sentiment 风险偏好变化 / positioning_squeeze 仓位挤压 / emotional_noise 情绪波动 /
technical_move 技术面波动 / no_clear_driver 无明显驱动

窗口级 confidence（0-1）：0.8-1.0 = 新闻与价格方向、时间、资产高度匹配；0.5-0.8 = 可能是主因之一；<0.5 = 归因不确定。

**首要原则：宁可保守，不要乱标。**
- 默认所有候选都是 noise。**只有当某条新闻能给出强而具体的因果链条**（"X 事件直接导致 symbol 价格 Y"），才标 primary_driver / secondary_driver。
- 模糊关联、宏观背景文章、与该 symbol 仅有间接联系的新闻 → 保持 noise；行情综述 / 事后总结 → post_hoc_explanation；对主驱动的重复转述 → secondary_driver（同事件簇）。**唯一例外**见下方 reference_changes 段：被跨资产签名确认的重大宏观 / 地缘**突发事件**，不算"间接联系"。
- 没把握时不给 primary_driver，market_reaction_type 按实际特征给 emotional_noise / technical_move / no_clear_driver —— 这份数据要拿去训练模型，假阳性会污染训练集。
- 下文条款中的「选入 / 不选」语义统一对应：**选入 = 标 primary/secondary_driver；不选 = 保持 noise 或标 post_hoc_explanation / contradictory**。
...
- **对标不可用**：当 reference_changes **全部为 null**，或**主力对标（纳指 / 原油 / 黄金）为 null、仅剩美元指数 / 美债10Y 等低波动品种基本走平**——典型时段：北京时间凌晨 05:00-06:00 的 CME 日休、周末 / 假日（加密窗口高发）——跨资产签名**不可用：既不能用来确认，也绝不能用来排除**。美元指数或收益率走平**不构成**"没有宏观 / 地缘事件"的证据；此时 BTC 等 7×24 资产往往是突发事件**唯一**的即时反应者。处理方式：退回**纯事件判断**——只有当候选里存在**量级重大**的硬事件（军事打击 / 开战 / 重大制裁或监管落地，而非言论、分析、回顾），其发布时间与窗口起点或加速段**分钟级吻合**、方向一致时，才选入；达不到这个门槛就 no_clear_news=true。
...
2. 再扫一遍候选新闻，给非 noise 的条目标注 causal_role——第 1 步判为共振时，重点核对宏观 / 地缘 / 政策类突发事件；判为独立行情时，只考虑该资产专属新闻；行情综述 / 收盘总结标 post_hoc_explanation，方向相反的消息标 contradictory；
3. 没有 primary_driver → market_reaction_type 按窗口特征给 emotional_noise / technical_move / no_clear_driver，summary 写"无明显因果新闻"；
4. 有 primary_driver → 给出对应的 market_reaction_type 与 confidence，summary 用一句话写清楚因果链条。

只返回 JSON，不要 Markdown 标记，不要解释性正文。格式：
{
  "news_roles": {"<新闻id>": "primary_driver|secondary_driver|amplifier|post_hoc_explanation|contradictory", ...},   // 只列非 noise 的候选；id 必须真实存在于候选列表
  "market_reaction_type": "fundamental_repricing|policy_expectation_shift|liquidity_shock|risk_sentiment|positioning_squeeze|emotional_noise|technical_move|no_clear_driver",
  "confidence": 0.85,                // 0-1
...
对**每个**窗口分别完成：给候选新闻标注**因果角色（causal_role）**，并判断该窗口的**市场反应类型（market_reaction_type）**与置信度。

**标签体系**：

每条新闻的 causal_role（每个窗口的输出里**只列非 noise** 的候选，未列出的一律默认 noise）：
- primary_driver 主驱动：直接触发该窗口异动的事件
- secondary_driver 次驱动：辅助驱动，或主驱动同一事件簇的补充 / 后续报道
- amplifier 放大器：放大既有趋势但非起因
- noise 噪音：与该窗口异动无关（默认值，**不要写进输出**）
- post_hoc_explanation 事后解释：价格先动、新闻找理由（"X 短线下挫，因 Y"类行情综述、收盘总结）
- contradictory 方向矛盾：新闻方向与价格反应相反（如缓和消息发布后价格继续下跌）
...
窗口级 confidence（0-1）：0.8-1.0 = 高度匹配；0.5-0.8 = 可能是主因之一；<0.5 = 归因不确定。
...
**首要原则：宁可保守，不要乱标。**
- 默认所有候选都是 noise。**只有当某条新闻能给出强而具体的因果链条**（"X 事件直接导致 symbol 价格 Y"），才标 primary_driver / secondary_driver。
- 模糊关联、宏观背景、与该 symbol 仅有间接联系的新闻 → 保持 noise；行情综述 / 事后总结 → post_hoc_explanation；对主驱动的重复转述 → secondary_driver（同事件簇）。**唯一例外**见下方 reference_changes 段：被跨资产签名确认的重大宏观 / 地缘**突发事件**，不算"间接联系"。
- 没把握时不给 primary_driver，market_reaction_type 按实际特征给 emotional_noise / technical_move / no_clear_driver —— 这份数据要拿去训练模型，假阳性会严重污染训练集。
- 不要为了"每个窗口都给点东西"而硬标 —— 一个 batch 里大多数窗口可能都没有 primary_driver，这是正常的。
- 下文条款中的「选入 / 不选」语义统一对应：**选入 = 标 primary/secondary_driver；不选 = 保持 noise 或标 post_hoc_explanation / contradictory**。
...
2. 再扫一遍该窗口的候选新闻，给非 noise 的条目标注 causal_role——第 1 步判为共振时，重点核对宏观 / 地缘 / 政策类突发事件；判为独立行情时，只考虑该资产专属新闻；行情综述 / 收盘总结标 post_hoc_explanation，方向相反的消息标 contradictory；
3. 没有 primary_driver → market_reaction_type 按窗口特征给 emotional_noise / technical_move / no_clear_driver，summary 写"无明显因果新闻"；
4. 有 primary_driver → 给出对应的 market_reaction_type 与 confidence，summary 用一句话写清楚因果链条。
...
只返回 JSON：
{
  "items": [
    {
      "window_id": int,
      "news_roles": {"<新闻id>": "primary_driver|secondary_driver|amplifier|post_hoc_explanation|contradictory", ...},   // 只列非 noise；id 只能来自该 window 自己的 candidates
      "market_reaction_type": "fundamental_repricing|policy_expectation_shift|liquidity_shock|risk_sentiment|positioning_squeeze|emotional_noise|technical_move|no_clear_driver",
      "confidence": 0.85,                // 0-1
      "summary": "不超过 80 字的归因结论；无明显驱动时写'无明显因果新闻'即可",
...
def _derive_compat_fields(roles: dict[int, str], reaction: str | None) -> tuple[list[int], bool]:
    """v2 roles → 旧消费方兼容字段：selected = primary+secondary（primary 在前）；
    no_clear = 无 primary_driver（或 reaction 显式 no_clear_driver）。"""
    selected = [nid for nid, r in roles.items() if r in ("primary_driver", "secondary_driver")]
    selected.sort(key=lambda nid: 0 if roles[nid] == "primary_driver" else 1)
    no_clear = not any(r == "primary_driver" for r in roles.values())
    if reaction == "no_clear_driver":
        no_clear = True
...
def _normalize_v2_labels(
    ...
    no_clear → market_reaction_type='no_clear_driver'。非法枚举值直接 ValueError。"""
    if request.news_roles is not None:
        ...
        roles = {
            int(nid): ("primary_driver" if i == 0 else "secondary_driver")
            for i, nid in enumerate(request.selected_news_ids)
        }
        reaction = request.market_reaction_type or ("no_clear_driver" if request.no_clear_news else None)
...
def _extract_v2_labels(data: dict, valid_ids: set[int]) -> AutoV2Parsed:
    ...
    selected, no_clear = _derive_compat_fields(roles, reaction)
    return AutoV2Parsed(roles, reaction, confidence, selected, no_clear, summary.strip()[:240])
...
def auto_annotate(session: Session, request: AutoAnnotateRequest) -> AutoAnnotateResponse:
    ...
    if not candidate_news:
        ...
        return AutoAnnotateResponse(
            ...
            no_clear_news=True,
            news_roles={},
            market_reaction_type="no_clear_driver",
            confidence=None,
...
def export_training_jsonl(session: Session, days: int = 365) -> list[str]:
    ...
    rows = (
        ...
        .order_by(NewsPriceAnnotation.window_end.asc())
        .all()
    )
    lines: list[str] = []
    for row in rows:
