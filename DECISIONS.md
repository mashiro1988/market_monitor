...
- 决策：两条规则取代三道闸——R1 检测基线只许取"目标时刻或更早"（任何边缘/空洞只低估不虚高）；R2 settle 写保护：cycle 只写「结束时间 ≤6h（WRITE_HORIZON）」的段 + 已存在未 settle 的行（停机恢复补 settle），历史一律只读；数据补洞后的修正走显式重算脚本，不靠扫描顺手改。
- 拒绝的备选：继续加边缘/空洞闸（永远堵不完所有边缘）；缩短扫描窗（不解决"部分视野可写"的根因）；每轮全历史重算（成本高且仍受洞影响）。
- 影响：behavior_segments._nearest 只往早取；classifier 三道闸代码删除，+WRITE_HORIZON_HOURS=6；48h 抓取保留（纯上下文水库，无写权）；gap_repair 补洞后的历史段修正 = 手动脚本（后续可挂自动触发）。
