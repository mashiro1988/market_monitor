...
| `GET /api/behavior/daily` | `behavior_views.daily_series`（每日最新 PIT 行优先；当日盘中按 `behavior_classifier.aggregate_day` 同口径现算 `live=true`；**方向拆分字段**（`up_net_sum`/`sent_up`/`sent_down`/`sent_*_net_sum`，2026-07-10 面板重画）由 `day_direction_extras` 对每一天 compute-on-read、不进 PIT——净幅由段原始数据决定、情绪归属按人工优先当前结论） | `BehaviorDailyResponse`（counts×tier×dir、三类构成（人工优先归并）+ `no_ref` 注记、涨/跌段净幅合计、情绪方向拆分） |
