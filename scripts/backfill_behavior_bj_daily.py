...
日聚合口径从 UTC 日改成北京日后，存档表里只剩旧的 date_basis='utc' 行。本脚本按北京日
重算最近 N 个**已结束**的北京日并写入 bj 行。旧 utc 行只读不动。幂等：目标日已有 bj 行则跳过。

⚠ 2026-08-06 起面板一律现算、不读这张表，跑本脚本**不会改变面板任何读数**。它现在只用于
补齐 class_version 换版基线（换版会就地重写段上的 classification，这些行是唯一的"变更前"底稿）。
