"""yfinance 源必须把所有资产组合并为**一次** yf.download 调用（fetch_history 唯一路径）。
...
1. 消除「单品种资产组」落入 len(ticker_list)==1 坏分支的结构性风险——
   ...
   是 DataFrame 而非 Series，旧代码把它当 Series 用：fetch_history 恒返回 0 条
   （回补永远补不出美元指数）。这是美元指数上线后一直无数据的确定性原因之一。
...
def test_single_ticker_multiindex_columns_handled(monkeypatch):
    ...
    静默 0 条——美元指数（currencies 组唯一品种）因此一直无数据。
    ...
    assert r.timestamp is not None and r.timestamp <= now


def test_history_single_symbol_failure_does_not_drop_batch(monkeypatch):
