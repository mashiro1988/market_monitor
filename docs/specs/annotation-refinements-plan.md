...
- **最猛 5min 段**：窗口区间内 |Δ%| 最大的那根 5min 的**起点时点** → payload `peak_move_start_bj / peak_move_pct`。这是真正的加速/触发时点，候选新闻离它越近权重越高。
...
2. **时间权重看最猛段、不是窗口起点**：真正的触发时点 = 窗口内**价格变动最猛烈那根 5min 的起点**（`peak_move_start_bj`）。候选新闻**离这个加速点越近权重越高**，尤其其临前。长窗口只是兜底：防①市场滞后反应（driver 在加速点前较久）②新闻源迟报（driver 在其后较久才推）。
