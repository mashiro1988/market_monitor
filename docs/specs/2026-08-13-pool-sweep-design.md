...
| 落库方式 | ~~直接立案+挂接~~ **(2026-08-15 修订:立案改提案确认制,见 §9;补挂仍自动)** | 原假设"事后审计代替事前签字"被两天实际使用推翻——删垃圾事件的成本高于点一次采纳 |
...
## §9 提案确认制(2026-08-15 修订,推翻 §2"直落库")

实际用了两天后用户裁定:"AI 立事件应该先让我确认——有很多我觉得没必要。"
立案回到**事前签字**,删除功能降级为兜底而非主审核手段:

- `POST /research/sweep` 只返回 `proposals[]`(名字/免闸词/成员 id + 标题摘要/一句话
  理由),**不再直接建事件**;补挂到现有事件的部分维持自动落——那些主题本就人工
  批准过,与 5min 挂接器同一权限,auto 链可摘可审,口径一致。
- 前端提案面板:每条提案一个勾选框(默认全选)+「成员快讯」可展开;
  「采纳选中并立案」→ `POST /research/sweep/apply`(**无状态**:勾选的提案原样传回,
  不在服务端暂存——单 worker 内存态重启即失,建表又过重),此时才走
  `create_event(created_from="sweep", link_source="auto", prompt_version)`;
  「全部忽略」一键收起,什么都不落。
- apply 校验:名字/成员非空、成员 id 去重、一次 ≤20 个、同名活跃事件跳过并报告
  (提案与采纳之间可能并发立案);`dry_run` 语义保留(连补挂都只计数不落)。
- 否决清单/进程锁/防幻觉逻辑不变;被删主题依旧不进提案。
