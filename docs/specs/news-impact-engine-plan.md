...
- **标注绑定 = A 策略（用户 2026-06-22 定，2026-06-28 简化冻结逻辑）**：① **只冻结「最新那一个」窗口**（`end == max(end)` 且 `end > now − 余量`）——它还在生长边缘、可能随新 bar 合并/延伸，故不可标；更早的窗口后面都已有更晚窗口 → 判「已走完」→ 可标（用户："只冻结最新的窗口就好了，逻辑清晰明了"）；② 标注冻结边界（`NewsPriceAnnotation` 已存 `window_start/end`）；③ 若之后 backfill 让某个**已标**窗口实质变化（边界挪 / 劈 / 并）→ 标「需复核」推给人，**不静默改、不静默丢**。台账反应侧不受影响（无人工绑定，保持 compute-on-read）。
...
- **A 有重要新闻、价没按预期动**（脱敏）。触发**只两件**（用户 2026-06-23 简化，**去掉"历史强弱"这道粗闸**）：该新闻 **a-priori 量级=大** 且 **被标为 noise**（= 结构化判定：当不上邻近任何窗口的 driver）→ 脱敏无反应标记。
  - **「无反应」是结构化判定，非超时阈值**（用户 2026-06-21 定）：看这条新闻**前后邻近**那些触发了阈值的价格窗口，它们的 driver 是哪几条；**只要这条量级大的新闻当不上其中任何一个窗口的 driver**，就判它「无反应」。代价：依赖 Phase 3 的窗口 driver 标注先完成；bootstrap 期由人工标。
  - **台账该主题最近几次反应 = 只作参考展示、不作门槛**（用户 2026-06-23：历史强弱"复杂了，只是给我一个参考"）。好处：topic 颗粒度问题（CPI vs 房价）对**判定**彻底无关——量级=大 这个逐条内容判当唯一主闸，参考看错也不误报。
  - 可选**描述**（非门槛）：参考里近期同主题都弱 → 标「渐进脱敏」（主题在死）；近期仍强、就这次弱 → 标「单次失灵」。附历史先例（该主题历史最强几次）供人审。
- **B 没新闻、价在动**（情绪）：窗口触发了但归因 = **无 driver**。**强/弱 = 该次振幅在最近 ~30 次情绪（无新闻）异动里的百分位**（非绝对阈值，可从历史回灌）；再叠加趋势方向（升势无故再涨=情绪强买点；跌势反之）。
- 防误报（简化后只剩一道）：**量级 a-priori = 大**（逐条内容判、不看价格、免疫 topic 颗粒度）当 A 的唯一主闸。历史强弱不再当筛子、只作参考展示。
