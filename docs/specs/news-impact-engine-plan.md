...
### 窗口 · 单 15min
- **一个** 15min 窗口，触发用**振幅（窗口内收盘价高低差）≥ 阈值**，连续 15min **合并**。无 1h、无多尺度、无净门槛。
- 同时记 net（方向）与 range（强度）；range 大、net≈0 → 窗口状态标「双向博弈」。
- 数据限制：现仅存 5min 收盘价，"振幅" = 窗口内收盘价高低差（抓跨 bar 博弈，抓不到单根 5min 影线）。本期接受此限制，不改 OHLC 存储。
...
**取代：** causal_role → `driver / redundant / noise`（去 contradictory 与 post_hoc）；reaction_type 三分类退场（改由「有无 driver + topic」派生）；双档窗口(15m+60m)+净门槛 → 单 15min 振幅触发。
...
### Phase 2 — 窗口改单 15min 振幅【小】
- **改** `services/annotation_service.py:load_price_windows` + `config.ANNOTATION_WINDOW_SCALES`：删 60m 档与多尺度合并/净门槛；改振幅（收盘价高低差）触发 + 连续合并；记 net/range，标「双向博弈」。
- **改/删** `tests/test_annotation_window_scales.py`（多尺度用例 → 振幅触发用例）。
- **验证**：回放脚本在 6/10 夜重跑，确认横跳被压、真实波动被抓；阈值按结果定。
...
每个 Phase 执行前单独展开成 bite-sized TDD 细化 plan。Phase 1 与 Phase 2 互相独立，可并行；3 依赖 2，4 依赖 1，5 依赖 1+3。
