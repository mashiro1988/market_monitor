...
> Historical document: this spec describes the former Streamlit dashboard era. The current runtime is FastAPI + React/Vite at `http://localhost:8000`; use root `ARCHITECTURE.md`, `DATAFLOW.md`, and `README.md` as the live contract.
...
#### Polymarket（`scanners/sources/polymarket/`）

- **实际只使用 Gamma API**（`https://gamma-api.polymarket.com`）：market/event slug、tag 候选发现 + 概率获取（`outcomePrices` 字段）
- config 中的 `api_url: https://clob.polymarket.com` 是保留配置，当前 Polymarket 采集链路未调用 CLOB
- watchlist slug 支持 Gamma market slug 与 event slug；event slug 会展开其 `markets`
- tag 仅用于候选发现，按 volume 降序取前 5；过滤器要求 `min_volume=100000`，并排除体育、天气、娱乐、数字货币等噪音市场
- 当前 Polymarket 主题聚焦 Fed 利率、US inflation、Hormuz shipping normalization
...
- 数据窗口：可选 2小时 / 6小时 / 24小时 / 7天 / 30天，默认 24小时
- 顶部按问题族绘制 Yes 概率对比图，例如 Fed 2026 降息次数、通胀阈值、Hormuz 通行恢复截止日
- 按 (market_id, outcome) 去重取最新，作为 metric 展示
- 按 market_id 分组，每组显示各 outcome 的概率 metric
- 每个市场下方绘制各 outcome 的历史概率变化图
- 概率变化 ≥ 3% 的市场标记 🔥 并默认展开
- autorefresh：120 秒
