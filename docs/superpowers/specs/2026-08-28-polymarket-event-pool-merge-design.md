...
①池页「找市场提案」按钮(本线全部 active 事件);②事件详情单事件按钮
(event_id 参数);③已结算/断流市场卡「找后继」按钮(=该市场所挂事件的单事件
提案)。
...
- **事件详情加「市场定价」区块**:每个关联市场一张卡——当前概率大字 + 24h 变化 +
  概率曲线(窗口 24h/7 天/30 天/全部;原预测页 2h/6h 档在小时粒度下只剩 2-6 个点,
  删除)+ 交易量/到期日/已结算徽章 + 摘下按钮。新挂市场首轮采集前显示"等待首轮
  采集"占位(最长 1 小时)。
- 事件列表卡片加关联市场计数小徽章。
