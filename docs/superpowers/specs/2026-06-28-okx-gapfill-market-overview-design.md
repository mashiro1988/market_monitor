...
## 8. 前端：区分标注（**已选 B：休市阴影带**）

> 两轮评审显示 A（双线）反复踩 recharts 坑（颜色按下标移位、connectNulls 全局、图例翻倍、tooltip）。**用户已选 B**（改动小、零回归）；A 留作未采用备选记录。
