...
4. 数据源异常对用户**显性可见**：市场概览卡片按品种标注 滞后/源中断/休市 状态
   （2026-07-22 用户新增需求），不再出现"源挂了半天、页面看不出来"。
...
## 5. 工作流三：卡片源状态标注（依赖工作流二的时段表）

### 5.1 判定（后端 `services/market_service.py` + `scanners/market_sessions.py`）

`/api/market/latest` 的每个 `MarketLatestItem` 新增两个字段：
`freshness: "live" | "stale" | "source_down" | "closed"`、`stale_minutes: int | null`。
判定在**后端**完成（复用工作流二的时段表，夏令时口径唯一；前端不自算时差，
规避 timestamp_utc naive 解析坑）：

- 品种当前**不在交易时段** → `closed`。
- 在时段内，最新 bar 距今 ≤ STALE 阈值 → `live`（无标注）。
- 在时段内，滞后 ∈ (STALE, DOWN] → `stale`，附滞后分钟数。
- 在时段内，滞后 > DOWN 阈值，**或**该品种所属源在最近一轮扫描报错
  （单进程内存态 `run_scan_once.last_source_statuses`，ok=False）→ `source_down`。

阈值进 config（用户可调）：`FRESHNESS_STALE_MINUTES=15`、`FRESHNESS_DOWN_MINUTES=60`。

时段表覆盖面为此扩展（仅用于标注判定，不影响采集）：
crypto/perp = 24×7；美债 = America/New_York 周日 18:00 → 周五 17:00（每日 17:00–18:00 跳过）；
日债 = Asia/Tokyo 09:00–11:30、12:30–15:00。债券时段为近似口径（只影响标注边缘时刻的
灰/黄判定，不影响数据），在模块注释中注明。

### 5.2 呈现（`frontend/src/pages/MarketPage.tsx`）

沿用卡片既有 badge 形态（`badge-proxy` 先例）：

| freshness | 标注 | 悬停说明 |
|---|---|---|
| live | 无 | — |
| stale | 黄标「滞后 X 分钟」 | 开市中但最新 K 线滞后，可能源被限流 |
| source_down | 红标「源中断」 | 数据源持续无返回/报错，当前显示为最后可得旧价 |
| closed | 灰标「休市」（低调样式） | 该市场当前不在交易时段 |

前端仅按后端字段渲染，不做任何时间计算。`types.ts` 同步镜像新字段。
...
- 回补脚本：CSV 往返（写→读→构造 PriceRecord）、行级校验、dry-run 不落库、
  幂等重复导入零新增。
- 卡片标注：freshness 判定单测（时段内/外 × 滞后档位边界、源报错直判 source_down、
  crypto 24×7、债券近似时段）；API 响应 schema 含新字段；前端按 freshness 渲染
  四态的组件测试（MarketPage.test.tsx）+ `npx tsc --noEmit`。
...
3. 卡片源状态标注：依赖治本的时段表，同分支紧随实施（后端字段 → 前端渲染），
   随治本一并部署（含 `frontend-build`）。
4. 解封判定探测：服务器 venv 内 curl_cffi Chrome 会话请求
...
- 治本：测试全绿；上线后 journalctl 连续 24h 无 `YFRateLimitError` 复发
  ...
  每日请求量按 §4.3 口径下降 ≥50%。
- 卡片标注：开市中人为断源（本地环境模拟）→ 对应卡片先黄后红；休市品种显示灰标；
  正常品种无标注；线上部署后用当时真实市况抽查一轮。
