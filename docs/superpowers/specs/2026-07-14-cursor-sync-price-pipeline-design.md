...
`period="7d"` 和 `period="1d"` 都是每品种一次 chart 请求（yf.download 无批量端点，见 `_all_tickers` docstring）。
现状每个 5min 周期实际下载**两遍**全品种（`fetch()` 一遍 + 滚动追平 `fetch_history` 又一遍），
加上每小时 gap_repair 再一遍：约 16 品种 × (2×288 + 24) ≈ **9,600 请求/天**。
新路径每周期一遍 + 每小时监控一遍：16 × (288 + 24) ≈ **5,000 请求/天，近乎减半**——
本案本身就是对"yfinance 间歇限频"的最有效缓解。若仍见 429，后续旋钮：
`yf.download(threads=False)` 串行化消突发（指纹会话 curl_cffi 已在用）；最终手段 = 付费源（另案）。
...
> OKX 窗口通俗解释：每轮问一句"BTC 和 ETH 谁的库存进度最落后？"，从那个时刻再往前 30min 开始拉到现在
> （进度快的品种多拉的部分入库时幂等跳过）；无论停机多久，起点最早不超过 72h 前（更早历史放弃，
> 与现行上限一致）。BTC/ETH 同为 24/7 品种实际总是同步，"最落后"只是兜底万一某品种单独漏了一根。
> 实现就是一行 `max(min(各品种库内最新时刻) − 30min, now − 72h)`，不是新机制。
