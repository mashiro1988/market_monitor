...
**两条并行的数据摄入路径**：
1. **5min 扫描循环**（游标同步，2026-07-14 重构）：`run_scan_once()` -> price scanner **窗口同步**（每源一个窗口 `max(now−CAP, min(游标−30min, now−24h))`，`fetch_history` 拉取 + 幂等入库——采集即回补，无独立补数机制）-> news/prediction scanner -> SQLite -> alert -> 新闻滚动回补 + LLM 内容打标（`tag_untagged`，原每小时 job 收编）。
2. **远程数据周期**（独立 APScheduler job）：`remote_data_cycle` -> SFTP 拉 BMAC pkl -> sector_scanner 算板块涨跌 -> `sector_returns` 表。两条路径用各自的 `max_instances=1` 锁，互不阻塞。
3. **价格行为引擎周期**（2026-07-09）：`behavior_cycle` 每 5min（错峰 +2min 让快照先落库）——`behavior_classifier.classify()` 段检测 + upsert `behavior_segments` + settle（段止+后窗1h+余量）后 S×新闻十字格分类；`behavior_daily_summary` UTC 00:05 把昨日聚合 **append** 进 `behavior_daily_summaries`（point-in-time，读取取最新）。
...
| `api/` | `app.py`、`routes.py`、`deps.py`、`errors.py` | FastAPI 工厂、约 25 条路由、DB 依赖、统一错误响应；lifespan 里启动扫描 / 远程数据 / cmc_bootstrap（缺口自愈 job 已退役，2026-07-14 游标同步）。 |
...
| `python run.py scan` | 单次扫描周期（价格窗口同步 -> 新闻 -> 新闻滚动回补 + 打标 -> 预测 -> 告警）。 | `run.py` |
...
1. CLI 分发 -> `execute("scan")` -> `run_scan_once()`（`services/scan_runtime.py`）
2. 获取跨进程文件锁 `_scan_lock()`（`services/scan_runtime.py`）
3. `PriceScanner().scan()` `scanners/price_scanner.py` -> **游标同步**：每源算窗口（游标=库内最落后品种最新 bar；yfinance CAP 7d / OKX CAP 72h / 至少回看 24h）-> `fetch_history` -> `_save_records` 幂等入库（返回本轮插入）-> gap_filler 休市合成点 -> `price_snapshots`。迟到 bar / 中段稀疏 / 停机重启统一由下一轮窗口自愈，**无独立补数机制**。
4. `NewsScanner().scan()` `scanners/news_scanner.py` -> Jin10 + CNBC RSS + InvestingLive + FinancialJuice -> 上一个已收口 5m 窗口 -> `NewsScorer.enrich_batch()`（DeepSeek）-> `_save_records` -> `news_items`
5. `_run_news_rolling_backfill()` + `_tag_new_news()`（`services/scan_runtime.py`）-> 新闻回补最近 2 个已收口窗口 + 给 `tagged_at IS NULL` 的新闻打内容标签（原每小时 job 收编，2026-07-14）
6. `PredictionScanner().scan()` `scanners/prediction_scanner.py` -> Polymarket Gamma -> `_save_records` -> `prediction_markets`（同时把上一条 DB 记录的概率写入 `prev_probability`）
7. `AlertEngine().evaluate_all(...)` `alerts/engine.py` -> price_change / price_level / news_importance / prediction_shift -> 查 `alert_logs` 做冷却 -> `_dispatch` 到 console + WeCom -> 落 `alert_logs`（price_records=本轮插入，staleness 保护跳过追平轮历史 bar）
