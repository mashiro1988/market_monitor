...
> 本地单用户宏观市场监控台。Python 扫描器 + 告警写 SQLite；FastAPI 在 `http://localhost:8000` 上服务 React/Vite SPA。最近一次基于代码扫描确认：2026-07-09（**价格行为引擎落地**，spec=`docs/specs/volume-behavior-engine-discussion.md` v0.4、plan=`docs/specs/price-behavior-engine-plan.md`：新服务 `behavior_segments` 三档段检测（语义照搬 `_scale_events`）、`resonance_score` 共振分 S/ESS/rolling S、`behavior_classifier` settle 后 S×新闻十字格分类 + PIT 日汇总（APScheduler `behavior_cycle` 5min + `behavior_daily_summary` UTC 00:05）、`behavior_views` 读层、`behavior_calibration`+`scripts/behavior_calibrate.py` 校准四件套；新表 `behavior_segments`/`behavior_daily_summaries`；新端点 `/api/behavior/{segments,daily,linkage}`；前端新页 `/behavior`；auto-annotate prompt **`v11-20260709`**——payload 用 `s_scores/max_ref/sync_ref_count/machine_class` 取代 ±1h Pearson `correlations`，训练导出 window 同步换 `s_scores`；标注窗口源开关 `BEHAVIOR_REPLACES_ANNOTATION_WINDOWS` 默认关；`price_snapshots` retention 30→90d。承接 2026-07-02 状态：标注 Phase3a 三角色、宏观对标带本身三段、代理 `config.py:32` import-time 探测）。
>
